...
// technically this should involve DAML.Renderers.Executable(PBlock, Rendlet) or something
...
DAML.Compiler.empty_PBlock = function() {
  return new DAML.PBlock(DAML.Compiler.wrap_in_empty_ABlock([]))
}
...
DAML.Parser.wrap_in_empty_ABlock = function(body) {
  return new DAML.ABlock([], body)
}
...
DAML.ABlock = function(head, body) {
  // soooooo... this assumes head is a bunch of segments and body is a bunch of strings or ABlocks. right. gotcha.
  head = head || false
  body = body || false
  ...
  if(body) {
    // ensure it's an array of strings and ABlocks
  }
...
DAML.PBlock = function(ABlock, space, parent) { // output var name?
  ...
  this.string = ABlock.string
  this.body = ABlock.body
  ...
  // what's in the body?
  if(ABlock.body) {
    this.body = ABlock.body.map(function(item) {return DAML.Compiler.make_block(item, space, this)})
  }
}

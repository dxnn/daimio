...
DAML.run = function(daml, space) {
  ...
  return DAML.SPACES.top.execute(DAML.string_to_ABlock(daml))
...
DAML.string_to_ABlock = function(string) {
  return DAML.string_to_ABlock_and_RendLet(string)[0]
}

DAML.string_to_ABlock_and_RendLet = function(string) {
  var P = DAML.Parser
    , RL = 12345
    , AB = P.wrap_in_empty_ABlock(
...
DAML.Compiler.empty_PBlock = function() {
  return new DAML.PBlock(DAML.Parser.wrap_in_empty_ABlock([]))
}
...
DAML.Parser.split_block = function(basic_block) {
  ...
  // either {head: "{begin foo | string reverse}", body: ["lala"]}
  // or     {head: "{begin foo | string reverse}", body: ["la", {head: {Fancy: {glyph: "$", rest: "x"}}, body: false} "la"]}
  // or     {head: "{5 | math add to 3}"}, body: false} --> no body
...
DAML.Parser.parse_pipeline = function(block_wrapper) {
  ...
  var pipeline_string = block_wrapper.head
    , output = []
    , P = DAML.Parser
...
DAML.Space.prototype.execute = function(ablock, callback) {
  ...
  if(pblock.pipeline) {
    pblock.pipeline.execute(callback)
  } else {
    DAML.continue_block(pblock.body, '', callback)
  }
}
...
DAML.continue_block = function(body, output, callback) {
  ...
  if(typeof item == 'string') {
    ...
    return DAML.continue_block(body, output + item, callback)
  }
  ...
  if(!body.length) {
    ...
    new_cb = function(result) {
      DAML.continue_block(body, output + result, callback)
    }
  }

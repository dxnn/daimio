...
DAML.run = function(daml, space) {
  ...
  return DAML.compile(DAML.parse(daml), space).execute()
...
// TODO: the non-string return values idea is a leaky abstraction -- clean this up
...
/*
  ...
  head is either an Apipeline or a falsy value
  body is either an array of strings and Ablocks or a falsy value

  An Apipeline is an array of 
  ...
  Head is a PPipeline or a falsy value
...
// NOTE: this returns an ABlock
DAML.parse = function(string) {
  var P = DAML.Parser
  return P.wrap_in_empty_Ablock(
           P.split_string(string)
            .map(P.split_block)
            .map(P.parse_pipeline))
}

DAML.Parser.wrap_in_empty_Ablock = function(array) {
  return { block: 
           { head: ""
           , body: array
         }}

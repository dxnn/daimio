...
DAML.Block = function(string, space) {
  // default to top-level space
  
  // split string
    // single pipeline? build it
    // otherwise build all sub-blocks
}
...
// pipelines are just like people too
// 

DAML.Pipeline = function(string, dialect) {
...
DAML.Pipeline.prototype.next = function(cb) {
  var self = this
...
DAML.Pipeline.prototype.bound_next = function() {
  return this.next.bind(this)
} 

DAML.Pipeline.prototype.reset = function() {
  // THINK: this is probably a bad idea, but it makes debugging easier... can we reuse stacks?
  this.last_value = null
  this.pcounter = 0
} 

DAML.Pipeline.prototype.run = function() {
  var self = this
...
DAML.Process = function(pipeline, varset, when-done) {
  this.pcounter = 0
  this.varset = varset
  this.pipeline = pipeline
  this.when-done = when-done
  this.size = pipeline.stack.length
}

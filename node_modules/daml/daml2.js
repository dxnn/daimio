...
// primitive (number, string, block, list of primitives) <- is this anything??
...
DAML.add_type('template', function(value) {
  ...
  return DAML.blockify(value)
})

DAML.add_type('string_or_block', function(value) {
  if(DAML.isBlock(value)) return value
  return DAML.stringify(value)
})
...
// DAML.__compiler.make_block = function(params) {
...
//   
//   return DAML.blockify(string, ptree, fun)
// }
...
//   
//   if(DAML.isBlock(daml)) return daml // already block
//   
...
// DAML.parse = function(string, block_flag) {
...
//   // FIXME: this is really hacky, using block_flag instead
//   var this_was_a_string_thing_block_bing = false;
//   if(string.slice(-1) == String.fromCharCode(8)) {
//     string = string.slice(0, -1);
//     if(string.indexOf(DAML.command_open) !== -1) {
//       this_was_a_string_thing_block_bing = true;
//     }
//   }
//   
...
//   if(output.length > 1 || this_was_a_string_thing_block_bing) { // FIXME: really hacky
//     // if(DAML.in_block) {
...
//       output = DAML.wrap_command('__compiler', 'make_block', {string: old_string, ptree: output});
//     // }
//   } 
...
DAML.get_next_thing = function(string) {
  ...
  if(thing.command.slice(0,7) != DAML.command_open + 'begin ') return thing; // return if it's not a block
  ...
  end_end = DAML.command_open + 'end ' + block_name + DAML.command_closed;
...
DAML.param_parser = function(string) {
  ...
  // "..." => literal string (actually a block)
...
DAML.stringify = function(value) {
  if(DAML.isBlock(value)) return value.toString();
  
  if(typeof value == 'function') {
    if(DAML.isBlock(value())) return value().toString(); // handles wrapped blocks // THINK: is this enough?
    // THINK: this is really nasty, if the value() isn't a block, because it actually processes it twice.
  }
...
// DAML primitives are numbers, strings, lists, hashes (simple key/value pair objects), and blocks. boolean false is used internally to denote nasty values and failures, but generally converted to '' on output.
...
// DAML.get_block = function(string) {
//   var hash = murmurhash(DAML.stringify(string));
//   return DAML.BLOCKS[hash] ? DAML.BLOCKS[hash] : false;
// };
...
DAML.blockify = function(string, ptree, fun) {
  // if string is a block, return it
  if(DAML.isBlock(string)) return string;
  ...
  if(!DAML.BLOCKS[hash]) {
    DAML.BLOCKS[hash] = DAML.Block.spawn({}, string, ptree, fun);
  }
  
  return DAML.BLOCKS[hash];
};
...
DAML.parse = function(string) {
  var P = DAML.Parser
  return P.wrap_in_empty_block(
           P.split_string(string)
            .map(P.split_block)
            .map(P.parse_pipeline))
}
...
DAML.Parser.get_next_thing = function(string) {
  ...
  if(string.slice(0,7) != DAML.command_open + 'begin ')
...
DAML.Parser.split_block = function(basic_block) {
  ...
  // either {block: {head: "{begin foo | string reverse}", body: ["lala"]}} 
  // or     {block: {head: "{begin foo | string reverse}", body: ["la", {block: {head: {Fancy: {glyph: "$", rest: "x"}}}} "la"]}} 
  // or     {block: {head: "{5 | math add to 3}"}} --> no body
  
  if(block_string.slice(0,7) == DAML.command_open + 'begin ') { // regular block
    var end_match = block_string.match(/\{end (.?\w+)\}$/)
    if(!end_match) {
      DAML.setError("No end block found")
      return ""
    }
    block.name = end_match[1]
    block.head = DAML.Parser.get_next_thing(block_string.replace(/^{begin /, '{!!!!! '))  // TODO: yuckyuck
                            .replace(/^{!!!!! /, DAML.command_open + 'begin ')
    block.body = DAML.parse(block_string.replace(/\{end \w+\}$/, '')
                                        .slice(block.head.length))
  } 
  else { // single pipeline
    block.head = block_string
  }
...
DAML.Parser.parse_pipeline = function(block_wrapper) {
  if(typeof block_wrapper == 'string') 
    return block_wrapper
  
  var pipeline_string = block_wrapper.block.head
    , output = []
    , P = DAML.Parser
  ...
  output = P.split_on_terminators(pipeline_string)
  ...
  block_wrapper.block.head = output
  return block_wrapper
...
DAML.Parser.try_all_the_things = function(segment, inside_pipeline) {
  var P = DAML.Parser
    , things = [ P.try_number
               , P.try_string
               , P.try_list
  ...
  if(segment.trim)
    segment = segment.trim() // THINK: is there a better place for this?
               
  for(var i=0, l=things.length; i < l; i++) {
    if(typeof segment != 'string') 
      return segment
    segment = things[i](segment)
  }
...
DAML.Parser.try_number = function(segment) {
  return (+segment === +segment) ? {Number: +segment} : segment // NaN !== NaN
}
...
DAML.Parser.try_string = function(segment) {
  ...
  return DAML.Parser.munge_string(segment)
}

DAML.Parser.munge_string = function(string) {
  if(string.indexOf(DAML.command_open) == -1) 
    return {String: string}
  
  return DAML.parse(string.slice(1,-1))
  
  // var block = DAML.parse(string.slice(1,-1))
  // 
  // if(!block.length)
  //   return block // THINK: when would this ever occur??
  //   
  // // block is an array, but it really should be an actual block. this is stupid, and needs to be fixed.
  // // like, is there some way to ensure we always package block guts in a block? the whole thing is a little weird.
  // return DAML.parse(DAML.command_open + 'begin foo' + DAML.command_closed + string.slice(1,-1) + DAML.command_open + 'end foo' + DAML.command_closed)
}
...
DAML.Parser.try_fancy = function(segment) {
  var matches = segment.match(/^([^0-9A-Za-z"{(])(\w+)$/) // THINK: this is probably too restrictive... 
  if(!matches) 
    return segment
    
  return {Fancy: {glyph: matches[1], word: matches[2]}}
}
...
DAML.Parser.try_pipeline = function(segment) {
  ...
  return DAML.Parser.parse_pipeline({block: {head: segment}})
}
...
DAML.Parser.munge_command = function(string) {
  ...
  */
  ...
  if(items.length == 1)      
    return {Alias: items[0], string: string}  // {math}
  
  if(items.length == 2) {
    if(/^[a-z]/.test(items[1]))   
      return {Handler: items[0], Method: items[1]}  // {math add}
    else
      return {Alias: items[0], Params: {"!": P.try_all_the_things(items[1])}, string: string}  // {math 1}
  }
  
  if(!/^[a-z]/.test(items[2])) {  // {math add 1}
    // collect alias 
    command['Alias'] = items.shift() // OPT: these shifts are probably slow...
  }
  else {  // {math add value 1}
    // collect H & M
    command['Handler'] = items.shift()
    command['Method'] = items.shift()
  }
  ...
  while(items.length) {
    ...
    params[word] = P.try_all_the_things(value)
  }
...
DAML.Parser.wrap_in_empty_block = function(array) {
  return { block: 
           { head: ""
           , body: array
         }}
  ...
  for(var index=0, l=string.length; index < l; index++) {
    ...
    */
    ...
    if(we_are_matching) { // stop matching
      if(label) {
        var tempobj = {}
        tempobj[label] = string.slice(match_break, index) // off-by-one?
        output.push(tempobj)
      }
...
DAML.Parser.split_on_space = function(string) {
  return DAML.Parser.split_on(string, /[\s\u00a0]/, false)
}



DAML.Parser.transforms = function(ptree) {
  return DAML.Parser.transform_terminator(ptree)
}

DAML.Parser.transform_terminators = function(ptree) {
...
DAML.compile = function(maybeString) {
  if(typeof maybeString == 'string') 
    maybeString = DAML.parse(maybeString)
  
  var ptree = maybeString
  
  // outside is always a block
  return DAML.Compiler.make_block(ptree)
}

DAML.Compiler = {}

DAML.Compiler.make_block = function(maybePtree, space, parent) {
  if(!maybePtree.block)
    return DAML.Compiler.empty_block
  
  // ptree = DAML.Parser.transforms(maybePtree)
  
  return new DAML.Block(maybePtree.block, space, parent)
}
...
DAML.Compiler.empty_block = function() {
  return DAML.Compiler.make_block({block: {head: ""}})
}
...
DAML.Block = function(ptree, space, parent) { // output var name?
  ...
  this.string = ptree.string
  this.body = ptree.body
  
  // make a pipeline out of head
  if(ptree.head) {
    this.head = DAML.Compiler.compile_pipeline(ptree.head, this)
  }
  
  // what's in the body?
  if(ptree.body) {
    this.body = ptree.body.map(function(item) {return DAML.Compiler.make_block(item, space, this)})
  }
}

DAML.Block.prototype.param_prep = function(varset) {
  if(this.head && !this.body) return this.head.execute(varset)
  
  return this
}

DAML.Block.prototype.execute = function(varset) {
  if(this.head && !this.body) 
    return this.head.execute(varset)
  
  var body = this.body.execute(varset)
  if(!this.head)
    return body
  ...
  // 
  // if(this.pipeline) 
  //   output = this.pipeline.execute(output)
  // 
  // return output
}

DAML.Block.prototype.toString = function() {
  return this.string;
},

DAML.Block.prototype.toJSON = function() {
...
//   
//   toJSON: function() {
//     return this.string; // because nested blocks are jsonified in escaped form [oh, but not transfered that way through vars. shucks.] meaning, on output the block is quoted, not run. but in/out of vars, the block needs to retain its blockiness. if we use JSON.stringify for both, this gives troubles.
//   },
//   
...
// Pipelines are the basic unit of functionality
// they are always wrapped in a block
DAML.Pipeline = function(ptree, block) {
...
//  chunks = DAML.remove_pipes(chunks, dialect) // convert pipes and labels to proper command stack (fun, params, name)
  ...
  */
  ...
  for(var i=0, l=ptree.length; i < l; i++) {
    var test = false
      , segment = ptree[i]
...
DAML.Pipeline.prototype.execute = function(varset, when_done) {
  ...
  new Process(this, when_done, this.varset).execute()
}
...
DAML.Compiler.try_make_block = function(segment, space, parent) {
  if(segment.block) {
    var block = DAML.Compiler.make_block(segment, space, parent)
    return { fun: null
           , ins: {}
           , outs: []
           , params: [block]}
...
DAML.Process = function(pipeline, when_done, varset) {
  ...
  this.size = pipeline.commands.length
...
DAML.Process.prototype.next = function(cb) {
  ...
  var funitem = this.block.stack[this.pcounter]
  this.pcounter++
...
DAML.Space = function(parent) {
  if(parent)
    this.parent = parent
}

DAML.SPACES.top = new DAML.Space()


function asdf() {
  ding = {flags: [], stack: []}
  ding.stack[0] = {params: [100, function(pipeline) {return 1}], flags: {}, fun: function(p, q) {console.log(p, q, 100); return p + q} }
  ding.stack[1] = {params: [200, function(pipeline) {return pipeline.last_value}], flags: {}, fun: function(p, q) {console.log(p, q, 111); return p + q} }
  ding.stack[2] = {params: [300, function(pipeline) {return pipeline.last_value}], flags: {}, fun: function(p, q) {console.log(p, q, 222); return p + q} }

  stack = new DAML.Pipeline(ding, function (val) {console.log(val, 12345)})
  
  
  block = {flags: [], stack: []}
  block.stack[0] = {params: [], flags: {async: true}, fun: function(cb) {
    var x = new DAML.Pipeline(ding, function (val) {cb(val)}) 
    x.run()
  }}
  block.stack[1] = {params: [7200, function(pipeline) {return pipeline.last_value}], flags: {}, fun: function(p, q) {console.log(p, q, 111); return p + q} }
  block.stack[2] = {params: [7300, function(pipeline) {return pipeline.last_value}], flags: {}, fun: function(p, q) {console.log(p, q, 222); return p + q} }

  fpipeline = new DAML.Pipeline(block, function (val) {console.log(val, 777)})
}


// er...
// A Block has a command pipeline, that's part of it.
// the Pipeline takes a block and a cb, runs the block, then calls the cb with its value
// pipeline needs last_value, pcounter, next function... maybe that's it
// cb will usually be another pipeline's bound next fun, but at the top level it's run or something.

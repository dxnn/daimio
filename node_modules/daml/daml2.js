...
DAML.pparse = function(string, block_flag) {
...
DAML.Parser.split_block = function(basic_block) {
  ...
  if(block_string.slice(0,7) == "{begin ") { // regular block
    block.name = block_string.match(/ (.?\w+)\}$/)[1]
    block.head = DAML.Parser.get_next_thing(block_string.replace(/^{begin /, '{!!!!! '))
                            .replace(/^{!!!!! /, '{begin ')
...
DAML.Parser.parse_pipeline = function(basic_block) {
  if(typeof basic_block == 'string') 
    return basic_block
  
  var pipeline_string = basic_block.head
    , output = []
    , P = DAML.Parser
  ...
  return P.split_terminators(pipeline_string)
           .map(P.try_all_the_things)
...
DAML.Parser.try_all_the_things = function(segment) {
  var P = DAML.Parser
    , things = [ P.try_number
               , P.try_string
               , P.try_list
...
DAML.Parser.munge_string = function(string) {
  ...
  var block = DAML.Parser.parse(string)
  ...
  return DAML.Parser.parse('{begin foo}' + string + '{end foo}')
}
...
DAML.Parser.munge_command = function(string) {
  ...
  */
  ...
  if(items.length == 1)      
    return {Alias: items[0]}  // {math}
  
  if(items.length == 2) {
    if(!/[a-z]/.test(items[1]))   
      return {Handler: items[0], Method: items[1]}  // {math add}
    else
      return {Alias: items[0], Params: {"!": P.try_all_the_things(items[1])}}  // {math 1}
  }
  ...
  while(items.length) {
    ...
    var param = items.shift()
    params[word] = P.try_all_the_things(param)
  }
...
DAML.Parser.split_on = function(string, regex, label) {
  ...
  for(var index=0, l=string.length; index < l; index++) {
    char_matches = regex.test(string[index])
    ...
    if(we_are_matching) { // stop matching
      if(label) {
        var tempobj = {}
        tempobj[label] = string.split(match_break, index) // off-by-one?
        output.push(tempobj)
      }
      ...
      output.push(string.split(match_break, index))
...
DAML.Block = function(string, space, parent) { // output var name?
...
DAML.Block.prototype.execute = function() {
  if(this.pipeline && !this.blocks) return this.pipeline.execute()
...
DAML.Pipeline = function(string, block) {
  ...
  this.commands = chunks
  
}

DAML.Pipeline.prototype.execute = function(when_done) {

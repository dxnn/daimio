...
DAML.run = function(daml, space) {
  ...
  return DAML.PBlock_to_Process(DAML.ABlock_to_PBlock(DAML.string_to_ABlock(daml))).execute()
...
DAML.string_to_ABlock = function(string) {
  var P = DAML.Parser
...
DAML.ABlock_to_PBlock = function(ABlock) {
...
DAML.Parser.split_block = function(basic_block) {
  ...
  if(block_string.slice(0,7) == DAML.command_open + 'begin ') { // regular block
    ...
    block.body = DAML.string_to_ABlock(block_string.replace(/\{end \w+\}$/, '')
...
DAML.Parser.munge_string = function(string) {
  ...
  return DAML.string_to_ABlock(string.slice(1,-1))
}
...
DAML.PBlock_to_Process = function(PBlock, space, parent) {
  // if(typeof maybeString == 'string') 
  //   maybeString = DAML.ABlock_to_PBlock(DAML.string_to_ABlock(maybeString))
  // 
  // var PBlock = maybeString
  ...
  return new DAML.PBlock(maybePtree.block, space, parent)
}

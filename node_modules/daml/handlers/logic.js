    ...
    methods: {
      ...
      'if': {
        ...
        fun: function(value, then, _else, _with, prior_starter, process) {
          var branch = DAML.ETC.isFalse(value) ? _else : then
          
          if(!_with)
            return branch
          
          if(branch.constructor == DAML.Segment) { // TODO: remove me when "block|anything" is supported
            branch = DAML.TYPES['block'](branch)
          }
        ...
        fun: function(value, prior_starter) {
         ...
         var my_tramp_prior_starter = function(bool) {
          ...
          var processfun = function(item, tramp_prior_starter) {
            ...
            if(found === count) {
              if(typeof item == 'function') 
                return item(function(value) {my_tramp_prior_starter(value)}) 
              else
                return item
            }
            ...
            if(typeof item == 'function') 
              var bool = item(function(value) {my_tramp_prior_starter(value)}) 
            else
              bool = item
          ...
          var joinerfun = function(total, value) {
            if(DAML.isNice(total)) return total
            if(DAML.isNice(value)) return value
            return null
          }
          
          return DAML.dataTrampoline(value, processfun, joinerfun, prior_starter)
          
          
          // var unwrapped = _.find(value, function(item) {
          //   return (typeof item != 'object' || DAML.isBlock(item))
          // })
          // 
          // if(unwrapped) {
          //   var new_value = []
      ...
      not: {
        ...
        fun: function(value) {
          return DAML.ETC.isFalse(value) ? true : false

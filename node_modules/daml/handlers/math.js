    ...
    methods: {
      ...
      add: {
        ...
        fun: function(value, to) {
          return DAML.ETC.Math.solver(value, to, function(a, b) {return a + b;});
        },
      },
      ...
      multiply: {
        ...
        fun: function(value, by) {
          return DAML.ETC.Math.solver(value, by, function(a, b) {return a * b;});
        },
      },
      ...
      subtract: {
        ...
        fun: function(value, from) {
          return DAML.ETC.Math.solver(from, value, function(a, b) {return a - b;});
        },
      },
      ...
      divide: {
        ...
        fun: function(value, by) {
          return DAML.ETC.Math.solver(value, by, function(a, b) {
            if(!b) 
              return DAML.setError('Division by zero is a crime against nature') || 0
            return a / b
          });
        },
      },
      ...
      mod: {
        ...
        fun: function(value, by) {
          return DAML.ETC.Math.solver(value, by, function(a, b) {
            if(!b) 
              return DAML.setError('Modulation by zero is a crime against nature') || 0
            return a % b
          })
        },
      },
      ...
      pow: {
        ...
        fun: function(value, exp) {
          ...
          if(value < 0 && exp % 1)
            return DAML.setError('Roots of negatives are not real') || 0
...
DAML.ETC.Math.solver = function(value, to, fun) {
  // TODO: we don't need this if the type is "array|number"
  value = (typeof value == 'object') ? DAML.toArray(value) : value
  to = (typeof to == 'object') ? DAML.toArray(to) : to
  ...
  // THINK: maybe wrap these with DAML.ETC.toNumeric to keep out NaNs
  if(arrays == 2) return DAML.ETC.Math.doubleArray(value, to, fun);
  if(arrays == 1) return DAML.ETC.Math.singleArray(value, to, fun);
  if(arrays == 0) return DAML.ETC.Math.naryanArray(value, to, fun);
};

DAML.ETC.Math.doubleArray = function(value, to, fun) {
  return value.map(function(val, key) {
    return fun(DAML.ETC.toNumeric(val), DAML.ETC.toNumeric(to[key]));
  });
};

DAML.ETC.Math.singleArray = function(value, to, fun) {
  ...
  if(DAML.ETC.isNumeric(to)) {
    return value.map(function(val) {
      return fun(DAML.ETC.toNumeric(val), DAML.ETC.toNumeric(to));
    });
  }

  // just the one array
  var total = false;
  value = DAML.toArray(value);
  for(var i=0, l=value.length; i < l; i++) {
    ...
    if(total === false) total = DAML.ETC.toNumeric(value[i]);
    else total = fun(total, DAML.ETC.toNumeric(value[i]));
  }
  return total;
};

DAML.ETC.Math.naryanArray = function(value, to, fun) {
  if(!DAML.ETC.isNumeric(value)) {
    DAML.setError("That is not a numeric value")
    value = 0
  }
  if(!DAML.ETC.isNumeric(to)) {
    // DAML.ETC.toNumeric(value)
    to = 0
  }
  return fun(DAML.ETC.toNumeric(value), DAML.ETC.toNumeric(to));        
};

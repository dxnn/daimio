...
DAML.import_models({
  process: {
    ...
    methods: {
      ...
      debounce: {
        desc: "Run a fing if it hasn't been run recently",
        help: "This runs the fing 'time' milliseconds after the last time you call it. Note that it breaks out of program flow, like {process wait}, and also that all copies of a given fing are treated identically (i.e. if @a and @b are string-equivalent fings, they'll use the same timer).",
        params: [
          {
            key: 'value',
            desc: 'The fing to run',
            type: 'template',
            required: true,
          },
          {
        ...
        fun: function(value, time) {
          var hash = value.hash
          if(!hash) return DAML.onerror('Value must be a valid fing')

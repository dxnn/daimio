...
DAML.import_models({
  variable: {
    ...
    methods: {
      ...
      get: {
        desc: "Get a space variable's value",
        help: "An alias for {variable get path :foo} would be {$foo}",
        params: [
          {
            key: 'path',
            desc: "A space variable's path, like :foo or :foo.#1.baz",
            // desc: "A space variable's path, like :foo or :foo.#1.{:bat}.*.baz",
            type: 'string',
            required: true,
            falsy: false,
          },
        ],
        fun: function(path, callback, process) {
          var varset = process.space.varset
          return DAML.resolve_path(path, varset);
      ...
      set: {
        ...
        fun: function(path, value, callback, process) {
          var varset = process.space.varset
            , words = path.split('.')
          ...
          if(words.length == 1) {
            ...
            DAML.recursive_insert(varset, words, value);
          }
          ...
          */
          ...
          //     DAML.run(bindings[i]); // we set __ for each, so they always take the same value (even if the variable itself is edited by earlier bindings)

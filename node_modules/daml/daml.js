...
DAML.import_fancy('_', {
  eat: function(token) {
    ...
    // segment.value = segment.value.word
    // return [segment]
    token.type = 'String' // FIXME: HERP DERP BERP
    return [token] // this way we don't destroy the inputs and key and whatnot
...
DAML.add_type('block', function(value) {
  if(DAML.isBlock(value)) {
    ...
    return function(callback) {
      ...
      space.REAL_execute(DAML.ABLOCKS[value.value.id], callback) 
    }
  }
  else {
...
, toJSON: function(segment) {
    ...
    if(decorators) {
      // TODO: refactor this and make it more general
      return decorators[0].value // TODO: find the OriginalString type decorator, not just the first one
    }
...
, munge_tokens: function(L, token, R) {
    ...
    var new_tokens = new_token_sets.reduce(function(a,b) {return a.concat(b)}, [])
...
, munge_tokens: function(L, token, R) {
    var new_tokens = DAML.clone(DAML.ALIASES[token.value.word])
...
DAML.Space.prototype.REAL_execute = function(block, callback) {
  var process = new DAML.Process(this, block, callback)
    , self = this
...
DAML.Process = function(space, block, when_done) {
  ...
  this.varset = {} // process-level vars, like wiring, should be local to the process
}

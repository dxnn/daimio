...
DAML.Space = function(seed_id, parent) {
  ...
  this.subspaces.forEach(function(subspace, subspace_index) {
    var my_ports = self.filter(function(port) {return port.space = subspace_index})
    subspace.ports
      .filter(function(port) {return !port.space && !port.station}) // outer ports
      .forEach(function(port, port_index) {my_ports[index].pairup(port)})
  })
...
DAML.Space.prototype.get_state = function(param) {
  return typeof this.param != undefined ? this.param : this.seed.state.param
}
...
var seedid = DAML.spaceseed_add({dialect: {commands:{}, aliases:{}}, stations: [], subspaces: [], ports: [], routes: [], state: {}})

DAML.ExecutionSpace = new DAML.Space(seedid)
  ...
  for(var seedkey in seedlikes) {
    ...
    for(var key in subspaces) {
      ...
      for(var portkey in seedlikes[spacekey].ports) {
        var subport = seedlikes[spacekey].ports[portkey]
        newseed.ports.push({space: index, flavour: subport[0], settings: {thing: subport[1]}, name: subport[1]}) // oy vey
        port_key_to_index[spacekey + '.' + portkey] = newseed.ports.length // note 1-indexed
      }
    }

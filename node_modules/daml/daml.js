...
DAML.Parser.try_string = function(segment) {
  ...
  if(segment.indexOf(DAML.command_open) == -1)
    return new DAML.Segment('Stringzz', segment.slice(1, -1), [2])
  
  return new DAML.Segment('Fing', segment.slice(1, -1)) // Here 'Fing' is only to distinguish an *unprocessed* block from a Block segment with a pointer -- it only exists in this transitional phase.
}
...
DAML.TRANSFORMERS.ExtractValuesFromList = function(L, segment, R) {
  ...
  for(var i=0, l=segment.value.length; i < l; i++) {
    var inner_segment = segment.value[i]
    segment.inputs.push(inner_segment.key)
    new_segments.push(inner_segment)
  }

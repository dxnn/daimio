...
DAML.import_fancy('$', {
  eat: function(token) {
    ...
    var pieces = token.value.slice(1).split('.')
...
, munge_tokens: function(L, token, R) {
    ...
    last_replacement.first = token.first
...
, munge_tokens: function(L, token, R) {
    if(token.value == '__') {
      ...
      token.type = 'Command'
      token.value = 'variable get name -1 type :pipeline' // THINK: this is pretty weird
...
, munge_segments: function(L, segment, R) {
    ...
    */
    ...
    if(segment.first) { // first in our class
      new_key = -1
    }
    else {
    ...
    outputs.forEach(function(segment) {
      var pipe_index = segment.names.indexOf('__pipe__')
        , this_key = new_key
        , key_index
      
      if ( new_key !== -1     // not 'first'
        && pipe_index !== -1  // is piped
        && my_key != segment.inputs[pipe_index]) // not piped to pipevar
         this_key = segment.inputs[pipe_index]   // then keep on piping (mostly for aliases)
      
      while((key_index = segment.inputs.indexOf(my_key)) >= 0) {
        segment.inputs[key_index] = this_key
    ...
    if(!outputs.length && segment.first) {
      
      /* 
        oy. so maybe a 'passthru' command? maybe varset[-1] becomes varset[__] and we hit that instead? maybe...
...
, munge_tokens: function(L, token, R) {
    ...
    new_tokens[0].first = token.first
...
DAML.Space.prototype.execute = function(ablock_or_segment, scope, prior_starter) {
  var self = this
  ...
  if(this.processes.length && this.only_one_process) {
    ...
    var thunk = function() {
      var result = self.REAL_execute(block, scope, prior_starter)
      if(result === result)
  ...
  return self.REAL_execute(block, scope, prior_starter)
}

DAML.Space.prototype.REAL_execute = function(block, scope, prior_starter) {
  var self = this
    , result
  ...
  var process = new DAML.Process(this, block, scope, prior_starter)
...
DAML.Space.prototype.cleanup = function(pid) {
  var value = this.scrub_process(pid)
  this.run_listeners(value)
  // this.run_queue()
}
...
DAML.Space.prototype.scrub_process = function(pid) {
  ...
  for(var i=0, l=this.processes.length; i < l; i++) {
    if(this.processes[i].pid == pid) {
      ...
      return proc.last_value // this is stupid
    }
  }
}

DAML.Space.prototype.run_listeners = function(value) {
  if(value !== undefined) {
    for(var i=0, l=this.listeners.length; i < l; i++) {
      this.listeners[i](value) // call the registered listeners
    }
  }
}
...
DAML.Process.prototype.done = function() {
  ...
  if(this.asynced && this.prior_starter)
    this.prior_starter(this.last_value)
  else 
...
DAML.Process.prototype.next = function() {
  ...
  if(wiring[key]) {
    inputs = wiring[key].map(function(index) {return DAML.isNice(varset[index]) ? varset[index] : null}) // THINK: why null?
  }

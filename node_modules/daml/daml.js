...
DAML.run = function(daml, callback, space) {
  ...
  var new_cb = function(value) {
    if(DAML.isBlock(value)) {
      DAML.TYPES['block'](value)(callback) // pass the callback to the block's function
    } else {
...
, execute: function(segment, inputs, dialect, callback) {
    ...
    return DAML.dataTrampoline(inputs, DAML.execute_then_stringify, DAML.string_concat, callback)
...
DAML.dataTrampoline = function(data, processfun, joinerfun, when_done) {
  var result, keys = Object.keys(data), index = -1, size = keys.length, value
  
  var inner = function() {
    while(++index < size) {
      value = processfun(data[index], bounce, result)
      if(value !== value) return NaN // send stack killer up the chain
      result = joinerfun(result, value)
    }
    
    return when_done(result)
  }

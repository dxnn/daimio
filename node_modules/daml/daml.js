...
DAML.import_port_type('dom-on-change', {
  dir: 'in',
  outside_add: function() {
    var element = document.getElementById(this.settings.thing)
      , self = this
...
DAML.import_port_type('dom-on-submit', {
  ...
  outside_add: function() {
    ...
    var callback = function(event) {
      event.preventDefault() // THINK: not sure this is always desired...
      var ship = {}
...
/*
  ...
  If base doesn't contain a path segment we'll create it.
...
, execute: function(segment, inputs, dialect, prior_starter, process) {
    ...
    state[name] = DAML.deep_copy(inputs[0]) 
...
DAML.make_spaceseeds = function(seedlikes) {
  ...
  for(var seedkey in seedlikes) {
    ...
    for(var key in ports) {
      newseed.ports.push({flavour: ports[key][0], settings: {thing: (ports[key][1] || key)}, name: (ports[key][1] || key) }) // TODO: oh dear this should not be
    ...
    newseed.routes = 
      routes.map(function(route) {
        return [port_key_to_index[route[0]], port_key_to_index[route[1]]]
    ...
    newseed.routes
      ...
      .forEach(function(route) {
        return DAML.setError('Invalid route!')
      })

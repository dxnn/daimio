...
DAML.import_fancy(':', {
  eat: function(segment) {
    ...
    // return [segment]
    return [new DAML.Segment('String', segment.value.word)]
  }
})
...
DAML.run = function(daml, space) {
  ...
  return DAML.SPACES.top.execute(DAML.string_to_block_segment(daml))
...
DAML.Parser.create_ABlocks = function(basic_block) {
  ...
  if(block_string.slice(0,7) != '{begin ') {
    segments = DAML.Parser.string_to_segments(block_string)
  ...
  segments = DAML.Parser.string_to_segments(head, ablocks[0].id)
  new_ablocks = DAML.Parser.get_ablocks_from_segments(segments)
...
DAML.Parser.segments_to_block_segment = function(string) {
  var P = DAML.Parser
    , tokens = P.string_to_tokens(string)
    , segments = P.token_to_segments(tokens)
    , wiring = {}
  
  DAML.mungeLR(segments, DAML.TRANSFORMERS.Rekey)
  
  // TODO: refactor this into get_wiring or something
  for(var i=0, l=segments.length; i < l; i++) {
    var segment = segments[i]
    
    if(!segment.inputs) continue
    wiring[segment.key] = segment.inputs
  }
  
  var block = new DAML.ABlock(segments, wiring)
    , segment = new DAML.Segment('Block', {key: block.key})
  
  return segment
}

DAML.Parser.string_to_tokens = function(string, inside_pipeline) {
  var tokens = []
    , munger = ''
    , P = DAML.Parser
  ...
  tokens = P.split_on_terminators(string.slice(1,-1))
...
DAML.Parser.tokens_to_segments = function(tokens) {
  ...
  for(var key in DAML.SegmentTypes) {
    if(!DAML.SegmentTypes[key].munge_tokens)
      continue
    
    munger = function(L, segment, R) {
      if(segment.type != key) return [L.concat(segment), R]
      return DAML.SegmentTypes[key].munge_segments(L, segment, R)
    }
...
DAML.Parser.string_to_segments = function(string, replacement_for_begin) {
  return DAML.Parser.token_to_segments(DAML.Parser.string_to_tokens(tokens))
  ...
  //   tokens = DAML.mungeLR(tokens, DAML.TRANSFORMERS[key])
  // }
  // 
  // return tokens
}
...
DAML.SegmentTypes.Number = {
  try_lex: function(string) {
    return (+string === +string) ? {type: 'Number', value: string} : string // NaN !== NaN
  }
, token_to_segments: function(token) {
    return [new DAML.Segment('Number', +token.value)]
  } 
...
, token_to_segments: function(token) {
    return [new DAML.Segment('String', token.value)]
  }
, execute: function(segment) {
    return segment.value
  }
...
DAML.SegmentTypes.Block = {
  try_lex: function(string) {
    ...
    return {type: 'Block', value: string.slice(1, -1)}
  }
, token_to_segments: function(token) {
    return DAML.string_to_block_segment(token.value)
    // var ablocks = DAML.string_to_ABlocks(segment.value.slice(1,-1))
    // 
    // DAML.addABlocks(ablocks) // ugh derp
    // 
    // return new DAML.Segment('Block', ablocks[0]['id'])
  }
, execute: function(segment) {
...
DAML.SegmentTypes.List = {
  try_lex: function(string) {
    ...
    return {type: 'List', value: string.slice(1, -1)}
  }
, munge_tokens: function(L, token, R) {
    ...
    var items = DAML.Parser.split_on_space(token.value)
                    .map(DAML.Parser.string_to_tokens)
                    .reduce(function(a,b) {return a.concat(b)}, [])
    ...
    for(var i=0, l=items.length; i < l; i++)
      token.inputs.push(items[i].key)

    return [L, new_tokens.concat(token, R)]
  }
, token_to_segments: function(token) {
    return [new DAML.Segment('List', token.inputs)]
  }
...
DAML.SegmentTypes.Fancy = {
  try_lex: function(string) {
    ...
    if(regex.test(string)) 
      return {type: 'Fancy', value: string}
...
DAML.SegmentTypes.Pipeline = {
  try_lex: function(string) {
    ...
    return {type: 'Pipeline', value: string}
  }
, token_to_segments: function(token) {
    var segment = DAML.string_to_block_segment(token.value)
    return [segment.value.when = 'now']
  }
, execute: function(segment) {
...
DAML.Parser.try_command = function(string) {
  if(!/[a-z]/.test(string[0])) // TODO: move all regexs into a single constants farm
    return string              // THINK: what good is this regex?
  
  return DAML.Parser.munge_command(string)
}
  ...
  */
  ...
  if(items.length == 2) {
    if(/^[a-z]/.test(items[1]))   
      ...
      return new DAML.Segment('Alias', {word: items[0], params: {'__alias__': P.string_to_tokens(items[1])[0] }})  // {add 1}
  }
  ...
  while(items.length) {
    ...
    if(!items.length) {
      params[word] = new DAML.Segment('Null')
      continue
    }
    
    var value = items.shift()
    params[word] = P.string_to_tokens(value)[0] // THINK: is taking the first one always right?
  }
...
DAML.Space.prototype.execute_head = function(head, callback) {
  ...
  for(var i=0, l=pipeline.length; i < l; i++) {
    ...
    varset[segment.key] = segment.process()
  }

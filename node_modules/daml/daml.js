...
DAML.import_port_type = function(name, pflav) {
  if(DAML.PORTFLAVOURS[name])
    return DAML.setError('That port flavour has already been imported')
  
  if(!pflav || !pflav.dock)
    return DAML.setError('That port flavour is missing required properties')
  
  if(typeof pflav.dock != 'function')
    return DAML.setError("That port flavour's properties are invalid")
  
  DAML.PORTFLAVOURS[name] = pflav
  return true
}

DAML.add_port = function(name, space, flavour, settings) {
  var pflav = DAML.PORTFLAVOURS[flavour]
  
  if(!pflav)
    return DAML.setError('Port flavour could not be found')
  ...
  var gateway = Object.create(pflav)
  gateway.name = name
  gateway.flavour = flavour
  gateway.settings = settings
  
  return DAML.PORTS[name] = gateway
}

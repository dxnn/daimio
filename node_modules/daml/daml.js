...
DAML.eat_var_pieces = function(pieces, token) {
  if(!pieces.length)
    return []
  ...
  var path = new DAML.Token('List', pieces.join(' '))
    , pathfinder = new DAML.Token('Command', 'list pathfind')
    
  pathfinder.names = ['data', 'path']
  pathfinder.inputs = [token.key, path.key]
  
  return [path, pathfinder]
}
...
DAML.import_fancy('$>', {
  eat: function(token) {
    ...
    return [token].concat(DAML.eat_var_pieces(pieces, token))
  }
})
...
DAML.import_fancy('_>', {
  eat: function(token) {
    ...
    token.value = {type: 'pipeline', name: token.value.slice(2)}
...
, munge_segments: function(L, segment, R) {
  if(segment.type == 'space') // space vars have to be collected at runtime
    return [L.concat(segment), R]
  
  // TODO: only {_foo} and its ilk can't be wired. all {_foo | whatevs} should be pushed down to wiring...
  
  if(!segment.value.prevkey) // some pipeline vars have to be collected then too
    return [L.concat(segment), R]
...
//     return [L.concat(token), R]
    ...
    ACTUALLY...
      ...
      - what about the "{__}" case?   does [{PLACEHOLDER}], {0: [*in]} make sense? is this crazy?
...
, munge_segments: function(L, segment, R) {
    var my_key = segment.key
      , new_key = segment.prevkey || '*in'
    
    // handles cases like "{__}"
    if(!R.length && segment.prevkey == '*in') {
      segment.type = 'Variable'
      segment.value = {type: 'pipeline', name: '*in'}
      return [L.concat(segment), R]
    }
...
DAML.Space.prototype.dock = function(ship, station_id) {
  var block_id = this.seed.stations[station_id - 1]
    ...
    , scope = {"*in": ship} // TODO: find something better...
...
DAML.seedlikes_from_string = function(stringlike) {
  ...
  stringlike.split("\n").forEach(function(line) {
    ...
    if(action) {
      ...
      if(action == 'station') {
        this_seed.stations[action_name] = continuation
      }
    ...
    if(/->/.test(line)) { 
      ...
      line.split('->').forEach(function(part, index) {
        ...
        if(part[0] == '{') {
          ...
          this_seed.stations[fakename] = part
          part = fakename
        }
        ...
        else if(part.indexOf('.') >= 0) { // subspace...
          var spacename = part.split('.', 1)
...
DAML.make_spaceseeds = function(seedlikes) {
  ...
  for(var seedkey in seedlikes) {
    ...
    for(var key in stations) {
      newseed.stations.push(DAML.Parser.string_to_block_segment(stations[key]).value.id) // block id

...
DAML.Parser.create_ABlocks = function(basic_block) {
  ...
  if(block_string.slice(0,7) != '{begin ') {
    destructure_me = DAML.Parser.get_fully_processed_segments_from_a_string_oh_goodness(block_string)
    segments = destructure_me[0]
    extra_blocks = destructure_me[1]
    return [new DAML.ABlock(segments)].concat(extra_blocks)
  }
  
  // a named block
  var begin_match = block_string.match(/^\{begin (.?\w+)\b/)
  ...
  body_string = block_string.slice(begin_match[0].length, -1 * end_match[0].length)
  ...
  destructure_me = DAML.Parser.get_fully_processed_segments_from_a_string_oh_goodness(head, ablocks[0].id)
  segments = destructure_me[0]
  extra_blocks = destructure_me[1]
    // return [new DAML.ABlock(segments)].concat(extra_blocks)
  
  // return pipeline and ABlock array
  return [new DAML.ABlock(segments)].concat(ablocks).concat(extra_blocks)
}
...
DAML.Parser.get_fully_processed_segments_from_a_string_oh_goodness = function(string, replacement_for_begin) {
  ...
  for(var key in DAML.TRANSFORMERS) {
    var destructure_me = DAML.mungeLR(segments, DAML.TRANSFORMERS[key])
    segments = destructure_me[0]
    extra_blocks = destructure_me[1] || []
  }

  // segments = DAML.mungeLR(segments, DAML.TRANSFORMERS.List)
  // segments = DAML.mungeLR(segments, DAML.TRANSFORMERS.Terminator)
  // segments = DAML.mungeLR(segments, DAML.TRANSFORMERS.Fancy)
  
  return [segments, extra_blocks]
}
...
DAML.Parser.try_string = function(segment) {
  ...
  return new DAML.Segment('Fing', segment)
...
DAML.Parser.try_fancy = function(segment) {
  ...
  return [new DAML.Segment('Fancy', {glyph: matches[1], word: matches[2]})]
  ...
  */
  ...
  if(items.length == 2) {
    if(/^[a-z]/.test(items[1]))   
      ...
      return new DAML.Segment('Alias', items[0], {"!": P.try_all_the_things(items[1])[0] })  // {math 1}
  }
...
DAML.TRANSFORMERS.List = function(L, segment, R) {
  ...
  // If there's a Block in the list, pull it out
  for(var i=0, l=segment.value.length; i < l; i++) {
    var item = segment.value[i]
...
DAML.ABlock = function(head, body) {
  ...
  if(body) {
    ...
    if(body.some && !body.some(function(item) {return (typeof item != 'string') && !item.id}).length) {
      this.body = body.map(function(item) {return (typeof item == 'string') ? item : {block: item.id} }) // THINK: this 'block' bit is a bod block ref. should we use segments here instead?
    }
  }

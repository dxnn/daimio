...
DAML.add_type('list-or-nothing', function(value) {
  if(value === false)
    return false
  else
    return DAML.toArray(value)
...
DAML.run = function(daml, space, callback) {
  if(!daml) return ""
...
, execute: function(segment, inputs, dialect) {
    // var replacements = dialect.aliases[segment.value.word]
    // 
    // if(!replacements) {
    //   // THINK: warning? error?
    //   return ""
    ...
    //   // last_replacement.params = last_replacement.params || {}
    // 
    //   for(var i=0, l=segment.names.length; i < l; i++) {
    //     var key = segment.names[i]
    //       , value = segment.inputs[i]
    //       , lr_index = last_replacement.names.indexOf(key)
    //       , lr_position = lr_index == -1 ? last_replacement.names.length : lr_index
    //     
    //     if(key == '__pipe__') {  

...
DAML.Parser.create_ABlocks = function(basic_block) {
  ...
  if(block_string.slice(0,7) != '{begin ') {
    var segments = DAML.Parser.get_fully_processed_segments_from_a_string_oh_goodness(block_string)
    return [new DAML.ABlock(segments)]
  }
  ...
  return [new DAML.ABlock(segments)].concat(ablocks)
}
...
DAML.TRANSFORMERS.List = function(L, segment, R) {
  ...
  for(var i=0, l=segment.value.length; i < l; i++) {
    ...
    segment.ins[i] = index
    segment.value[i] = new DAML.Segment()
...
DAML.ABlock = function(head, body) {
  ...
  if(body) {
    ...
    if(body.some && !body.some(function(item) {return (typeof item != 'string') && !item.id}).length) {
      this.body = body.map(function(item) {return (typeof item == 'string') ? item : {block: item.id} })
    }
  }

...
DAML.add_type('fallback', function(segment) { // this is the default type parser
  return segment.value
})

DAML.add_type('string', function(segment) {
...
DAML.add_type('number', function(segment) {
  var value = segment.value
...
DAML.add_type('integer', function(segment) {
  var value = segment.value
...
DAML.add_type('anything', function(segment) {
  return segment.value // THINK: why?
})

DAML.add_type('primitive', function(segment) {
  return segment.value // THINK: what?
})

DAML.add_type('hash', function(segment) {
  return segment.value // THINK: who?
  
  // segment = DAML.defunctionize(segment)
  //   return typeof segment == 'object' ? segment : [segment] // TODO: this doesn't always return a proper JS 'hash', which is likely very bad. Find a better way.
})

DAML.add_type('list', function(segment) {
  return DAML.toArray(segment.value)
})

DAML.add_type('rawlist', function(segment) {
  return DAML.toArray(segment.value) // THINK: erp?
})

DAML.add_type('template', function(segment) {
  return segment.value // THINK: oy...
  
  // TODO: add some checking here to ensure segment isn't barfable.
  // return DAML.blockify(segment) 
})

DAML.add_type('string_or_block', function(segment) {
  return segment.value // THINK: herpderp
  
  // if(DAML.isBlock(segment)) return segment
  // return DAML.stringify(segment)
})
...
// TODO: make this string->ABlock into a Renderer
...
DAML.Parser.get_fully_processed_segments_from_a_string_oh_goodness = function(string, replacement_for_begin) {
  ...
  // TODO: find a cleaner way to do this (like, in a single loop instead of one for each Transformer)
...
// this always returns an ARRAY of segments!
...
DAML.Parser.try_string = function(segment) {
  ...
  return new DAML.Segment('Fing', segment) // THINK: why not String?
...
//     return new DAML.Segment('String', string)
//   
//   return DAML.string_to_ABlocks(string.slice(1,-1))
// }
...
DAML.Parser.try_pipeline = function(segment) {
  ...
  // TODO: convert this into a block reference, and somehow pass back the new ABlocks...
  // ugh seriously what are we going to do with those new ABlocks??
  // ok, actually, if it's a straight pipeline we'll just unroll it later, like a list.
  // if it's a Block (like "x{foo}y") then we're in trouble...

  // new DAML.Segment('Block', segments)
  return [new DAML.Segment('Pipeline', segment)]
  
  // return DAML.Parser.get_fully_processed_segments_from_a_string_oh_goodness({block: {head: segment}})
}
  ...
  */
  ...
  if(items.length == 2) {
    if(/^[a-z]/.test(items[1]))   
      ...
      return new DAML.Segment('Alias', items[0], {'__alias__': P.try_all_the_things(items[1])[0] })  // {add 1}
  }
  ...
  return new DAML.Segment(command.type, command.value, params)
}
...
DAML.TRANSFORMERS.ExtractParams = function(L, segment, R) {
  if(!segment.params) return [L.concat(segment), R]

  var typelist = ['List', 'Block', 'Pipeline', 'Fing', 'Fancy']
  ...
  for(var key in segment.params) {
    ...
    new_segment.outs || (new_segment.outs = [])
    segment.ins || (segment.ins = {})
    
    new_segment.outs.push(index)
    segment.ins[key] = index

    segment.params[key] = null
...
DAML.TRANSFORMERS.List = function(L, segment, R) {
  ...
  for(var i=0, l=segment.value.length; i < l; i++) {
    var item = segment.value[i]
    // if(item.type != 'List') continue
    if(typelist.indexOf(item.type) == -1) continue
    
    // TODO: refactor this bit... it's (semi)-common to Terminators also
    item.outs || (item.outs = [])
    segment.ins || (segment.ins = {})
    
    var index = L.length + '-' + i // THINK: this creates duplicate in/out labels, which stinks, but maybe is ok?
    item.outs.push(index)
    segment.ins[i] = index
    segment.value[i] = new DAML.Segment('Null')
    
    R.unshift(item, segment)
    return [L, R]
  }

  // If there's a Block/Fing/Pipeline in the list, pull it out
  // for(var i=0, l=segment.value.length; i < l; i++) {
  //   var item = segment.value[i]
  //   if(typelist.indexOf(item.type) == -1) continue
  //   
  ...
  //   segment.ins || (segment.ins = {})
  //   
  //   var index = L.length + '-' + i
  //   item.outs.push(index)
  //   segment.ins[i] = index
  //   
  //   R.unshift(segment)
  //   return [L.concat(item), R]
  // }
...
DAML.TRANSFORMERS.Pipeline = function(L, segment, R) {
  ...
  if(last_segment) {
    if(segment.outs) {
      last_segment.outs = segment.outs
    }
  }
...
DAML.TRANSFORMERS.Terminator = function(L, segment, R) {
  ...
  if(/^\|/.test(segment.value)) {
    var next = R[0]
      , prev = L[L.length - 1]
      , index = L.length // FIXME: this DOES NOT WORK with backtracking transformers
    ...
    if(next && prev) {
      ...
      prev.outs.push(index)
      next.ins['__pipe__'] = index
      next.params['__pipe__'] = null
...
DAML.Segment = function(type, value, params, ins, outs) {
  ...
  if(ins)
    this.ins = ins
  if(outs)
    this.outs = DAML.toArray(outs) // THINK: do we need toArray?
}
    ...
    for(var key in segment.params) {
      ...
      if(key == '__alias__') {
        ...
        for(var other_key in last_replacement.params) {
          if(last_replacement.params[other_key] === null) {
...
DAML.Dialect.prototype.connect_commands = function(L, segment, R) {
  ...
  // add paramlist, an ordered array of params
  // add inslist, an ordered array of ins
  segment.paramlist = []
  ...
  for(var index in segment.method.params) {
    ...
    if(params[key]) {
      ...
      if(ins['__pipe__'] !== undefined) {
        param_value = {type: 'Input', value: ins['__pipe__']}
      }
      piped = true
    }
    
    if(!param_value && ins[key]){
      param_value = {type: 'Input', value: ins[key]}
    }
...
DAML.Space.prototype.execute_head = function(head, callback) {
  ...
  for(var i=0, l=pipeline.length; i < l; i++) {
    ...
    for(var i=0, l=params.length; i < l; i++) {
      if(params[i] && params[i].type == 'Input') { // TODO: change 'ins' to just Input segments (!!!)
        params[i] = varset[params[i].value] // TODO: or null
      }
    }
    ...
    if(segment.method) {
      ...
      */
      ...
      for(var i=0, l=params.length; i < l; i++) {
        if(method_params[i] && !method_params[i].type && DAML.TYPES[method_params[i].type])
          typefun = DAML.TYPES[method_params[i].type]
        else
          typefun = DAML.TYPES.fallback
        
        if(typeof type != 'function') // THINK: do we really need this?
          typefun = function(segment) {return segment.value}
        
        
        if(params[i] && params[i].type) { // proper segment
          params[i] = typefun(params[i])
        }
        else if(method_params[i].fallback) {
          params[i] = typefun(method_params[i].fallback)
        }
        else if(method_params[i].required) {
          errors.push('Missing required parameter: ' + method_params[i].key)
        }
        else {
          params[i] = undefined // THINK: really?
        }
      }
      
      if(!errors.length) {
        value = segment.method.fun.apply(segment.method, params)
      } else {
        // TODO: ERRORS
        value = null
      }

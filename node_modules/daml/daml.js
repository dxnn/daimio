...
DAML.import_gateway_type('space-in', {
  sendMessage: function(value, callback) {
  ...
  addListener: function(callback) {
    DAML.setError('This gateway is only for receiving messages')
  }
})

DAML.import_gateway_type('space-out', {
  sendMessage: function(value, callback) {
    DAML.setError('This gateway is only for sending messages')
  },
  addListener: function(callback) {
    var space = DAML.SPACES[this.thing]
...
DAML.import_gateway_type('spaceblock', {
  sendMessage: function(value, prior_starter) {
    var space = DAML.SPACES[this.thing.space]
    ...
    return space.execute(this.thing.block, scope, prior_starter)
  },
  addListener: function(callback) {
    var space = DAML.SPACES[this.thing.space]
...
// Handed to Outer Space as @Server
DAML.import_gateway_type('server', {
  sendMessage: function(value, prior_starter) {
    var space = DAML.SPACES[this.thing.space]
...
//   var dialect = DAML.SPACES.outer.dialect
...
DAML.SPACES.outer = new DAML.Space(DAML.DIALECTS.top)
...
DAML.Process = function(space, block, scope, prior_starter) {
  ...
  if(this.varset['*in'] === undefined)
    this.varset['*in'] = "" // ha ha jk oh wait we need this
}
...
DAML.Process.prototype.done = function() {
  ...
  if(this.block.wiring['*out']) {
    ...
    if(outs.length == 1) {
      output = this.varset[outs[0]]
    } 
    else {
      output = []
      for(var i=0, l=outs.length; i < l; i++) {
        output.push(this.varset[outs[i]]) // THINK: sometimes array sometimes not is always weird
      }
    }
  } 

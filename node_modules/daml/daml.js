...
DAML.import_port_type('space-in', {
  ...
  add: function() {},
  dock: function(value) {
    var space = DAML.SPACES[this.settings]
    
    if(!space)
      return DAML.setError('Space could not be found')
    
    return space.deliver(value)
  }
})
...
// something about using {}s to map something... and vs or? it was really clever, whatever it was.


DAML.Space = function(id, dialect, state, ports, stations, parent) {
  var self = this
  ...
  stations = stations || []
  this.stations = stations.map(function(block_id) { return DAML.ABLOCKS[block_id] })

  ports = ports || []
  this.port_data = ports
  
  this.ports = ports
  .map(function(port) {
    return port.space == id 
         ? new Port(self, port.flavour, port.settings, this.stations[port.station], port.name, port.typehint)
         : false
  })
  ...
  this.wiring = ports.map(function(port) {return port.outs})
                     // .filter(function(outs) {return outs.length}) // have to retain indices...
  

  this.parent = parent || false // THINK: false is outer... but there can be only one?

...
, munge_tokens: function(L, token, R) {
    ...
    if(!token.value || typeof token.value != 'string') // TODO: remove me!
      return [L, R]
    ...
    if(items.length == 1) {  // {math}
      ...
      if(/^[a-z]/.test(items[1])) {  // {math add}
        ...
        token.type = 'Alias'
        token.value = {word: items[0], params: {'__alias__': DAML.Parser.string_to_tokens(items[1])[0] }}
      }
    ...
    while(items.length) {
      ...
      params[word] = DAML.Parser.string_to_tokens(value)[0] // THINK: is taking the first one always right?
    }

    token.value.params = params
    return [L.concat(token), R]
  }
, token_to_segments: function(token) {
...
DAML.Space.prototype.execute = function(ablock_or_segment, callback) {
  ...
  for(var i=0, l=segments.length; i < l; i++) {
    ...
    if(wiring[key]) {
      inputs = wiring[key].map(function(index) {return this.varset[index] || null})
    ...
    var value = type.execute(segment, inputs)

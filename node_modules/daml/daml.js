...
DAML.noop = function() {}
DAML.identity = function(x) {return x}
...
DAML.track_event = function(type, target, callback) {
  if(!DAML.ETC.events)
    DAML.ETC.events = {}
  
  if(!DAML.ETC.events[type]) {
    DAML.ETC.events[type] = {by_class: {}, by_id: {}}
  ...
  var tracked = DAML.ETC.events[type]
...
DAML.send_value_to_js_port = function(to, value) {
  try {
...
DAML.port_standard_exit = function(ship) { 
  var self = this
...
DAML.port_standard_pairup = function(port) { 
  this.pair = port
  port.pair = this
}

DAML.port_standard_enter = function(ship, process) {
  ...
  if(!this.station)
    return DAML.setError('Every port must have a pair or a station')
...
DAML.import_port_type = function(flavour, pflav) {
  ...
  if(typeof pflav.add != 'function')
    pflav.add = DAML.noop // noop, so we can call w/o checking
  
  if(typeof pflav.exit != 'function') 
    pflav.exit = DAML.port_standard_exit
  
  if(typeof pflav.outside_add != 'function')
    pflav.outside_add = DAML.noop
  
  if(typeof pflav.outside_exit != 'function')
    pflav.outside_exit = DAML.noop
  
  if(typeof pflav.pairup != 'function')
    pflav.pairup = DAML.port_standard_pairup
  
  if(typeof pflav.enter != 'function')
    pflav.enter = DAML.port_standard_enter
...
DAML.import_port_type('to-js', {
  dir: 'out',
  outside_exit: function(ship) {
    // this is very very stupid
    
    var fun = DAML.ETC.fun && DAML.ETC.fun[this.settings.thing]
    if(!fun)
      return DAML.setError('No fun found')
...
DAML.import_port_type('dom-on-click', {
  dir: 'in',
  outside_add: function() {
    var self = this
    DAML.track_event('click', this.settings.thing, function(value) {self.enter(value)})
  }
})
    
DAML.import_port_type('dom-on-blur', {
  dir: 'in',
  outside_add: function() {
    var self = this
    DAML.track_event('blur', this.settings.thing, function(value) {self.enter(value)})
  }
})
    
DAML.import_port_type('dom-on-change', {
  dir: 'in',
  outside_add: function() {
    var self = this
    DAML.track_event('change', this.settings.thing, function(value) {self.enter(value)})
  }
})

DAML.import_port_type('dom-on-submit', {
  dir: 'in',
  outside_add: function() {
    var self = this
    ...
    DAML.track_event('submit', this.settings.thing, callback)
  }
})
...
DAML.import_port_type('dom-set-text', {
  dir: 'out',
  outside_exit: function(ship) {
    ...
    if(this.element) 
      this.element.innerText = DAML.stringify(ship)
  },
  outside_add: function() {
    this.element = document.getElementById(this.settings.thing)
    
    if(!this.element)
      return DAML.setError('That dom thing ("' + this.settings.thing + '") is not present')
...
DAML.import_port_type('dom-set-html', {
  dir: 'out',
  outside_exit: function(ship) {
    // OPT: we could save some time by tying this directly to paint events: use requestAnimationFrame and feed it the current ship. that way we skip the layout cost between screen paints for fast moving events.
    if(this.element) 
      this.element.innerHTML = DAML.stringify(ship)
  },
  outside_add: function() {
    this.element = document.getElementById(this.settings.thing)
    ...
    if(!this.element.hasOwnProperty('innerHTML'))
      return DAML.setError('That dom thing has no innerHTML')
  }
})

DAML.import_port_type('dom-set-value', {
  dir: 'out',
  outside_exit: function(ship) {
    // OPT: we could save some time by tying this directly to paint events: use requestAnimationFrame and feed it the current ship. that way we skip the layout cost between screen paints for fast moving events.
    if(this.element) 
      this.element.value = DAML.stringify(ship)
  },
  outside_add: function() {
    this.element = document.getElementById(this.settings.thing)
...
DAML.import_port_type('dom-do-submit', {
  dir: 'out',
  outside_exit: function(ship) {
    if(this.element)
      this.element.submit()
...
DAML.import_port_type('socket-add-user', {
  dir: 'in',
  outside_add: function() {
    var self = this
...
DAML.import_port_type('socket-remove-user', {
  dir: 'in',
  outside_add: function() {
    var self = this
...
DAML.import_port_type('socket-in', {
  dir: 'in',
  outside_add: function() {
    var self = this
...
DAML.import_port_type('socket-out', {
  dir: 'out',
  outside_exit: function(ship) {
    if(socket)
...
DAML.import_port_type('in', {
  dir: 'in'
})

DAML.import_port_type('err', {
  dir: 'out'
  // TODO: ???
})

DAML.import_port_type('out', {
  dir: 'out'
})

DAML.import_port_type('up', {
  dir: 'up'
  // THINK: this can only live on a space, not a station
})

DAML.import_port_type('down', {
  dir: 'down',
...
DAML.import_port_type('exec', {
  ...
  exit: function(ship) { 
    ...
    this.space.execute(DAML.Parser.string_to_block_segment(ship.code), {secret: ship}) // TODO: ensure this is a block, not a string
  }
})
...
/// HACKHACKHACK
DAML.import_port_type('turtle-add', {
  dir: 'out',
  outside_exit: function(ship) {
    var mainsvg = document.getElementById('mainsvg')

    if(!mainsvg)
      return DAML.setError('Your svg is out of alignment')
    
    DAML.ETC.turtles = DAML.ETC.turtles || {} // le sigh
    
    if(!ship.user)
      return DAML.setError('There is no user in that ship')
      
    if(DAML.ETC.turtles[ship.user])
      return DAML.setError('You already have a turtle')
    ...
    var string = '<g id="turtlebox' + ship.user + '" transform="translate(200,200)"><rect width="30" height="50" id="turtle' + ship.user + '" style="fill:' + ship.color  + '"></rect></g>'
      , frag = DAML.string_to_svg_frag(string)
    
    el = mainsvg.appendChild(frag)
    
    DAML.ETC.turtles[ship.user] = el
  }
})

// ugh hack ugh
DAML.string_to_svg_frag = function(string) {
...
DAML.import_port_type('svg-add-line', {
  dir: 'out',
  outside_exit: function(ship) {
    var element = document.getElementById(ship.thing)
...
DAML.eat_fancy_var_pieces = function(pieces, token) {
  if(!pieces.length)
    return []
  ...
  var path = new DAML.Token('List', pieces.join(' '))
    , peeker = new DAML.Token('Command', 'list peek')
...
DAML.import_fancy = function(ch, obj) {
  ...
  DAML.FancyRegex = RegExp(Object.keys(DAML.FANCIES)
                                 .sort(function(a, b) {return a.length - b.length})
                                 .map(function(str) {return '^' + DAML.ETC.regex_escape(str) + '\\w'})
                                 .join('|'))
}
...
DAML.import_fancy('__', {
  eat: function(token) {
    ...
    if(token.value != '__' && token.value != '__in') {
      DAML.setError('Only __ and __in are allow to start with __')
      return []
    }

    return [token].concat(DAML.eat_fancy_var_pieces(pieces, token))
  }
})

DAML.import_fancy('_', {
  eat: function(token) {
    return DAML.eat_fancy_var(token, 'pipeline')
  }
})

DAML.import_fancy('$', {
  eat: function(token) {
    return DAML.eat_fancy_var(token, 'space')
  }
})

DAML.eat_fancy_var = function(token, type) {
  var pieces = DAML.Parser.split_on(token.value, '.')
...
DAML.import_terminator('→', { // send [old]
  eat: function(stream, state) {
...
DAML.import_models = function(new_models) {
  _.each(new_models, function(model, model_key) {
    if(!DAML.commands[model_key]) {
      DAML.commands[model_key] = model
    } else {
      _.extend(DAML.commands[model_key]['methods'], model['methods'])
    }
  })
}
...
DAML.add_type('block', function(value) {
  if(DAML.isBlock(value)) {
    ...
    return function(prior_starter, scope, process) {
      ...
      space = process ? process.space : DAML.ExecutionSpace
...
DAML.resolve_path = function(words, base, fun) {
...
DAML.Pathfinders = []
DAML.import_pathfinder = function(name, pf) {
  ...
  if(typeof pf.gather != 'function')
    pf.gather = DAML.identity // returns a list of all matched items
  
  pf.name = name
  
  DAML.Pathfinders.push(pf)
  // find returns a list of matching items, empty for none, null for N/A [or value/null, if amount is one]
}

DAML.import_pathfinder('list', {
  ...
  gather: function(value, key) {
    ...
    for(var i=0, l=key.length; i < l; i++) {
      output.push(DAML.peek(value, [key[i]] ))
    }
...
DAML.import_pathfinder('position', {
  keymatch: function(key) {
    if( (typeof key == 'string') && /#-?\d/.test(key) )
      return 'one'
  },
  gather: function(value, key) {
    var safe_value = typeof value == 'object' ? value : ("" + value).split('')
      , vkeys = Object.keys(safe_value)
      , position = +key.slice(1)
      , index = position < 0 ? (vkeys.length + position) : position - 1
      
    return [ safe_value[ vkeys[ index ] ] ]
  }
})

DAML.import_pathfinder('star', {
  keymatch: function(key) {
    if(key == '*')
      return 'many'
  },
...
DAML.peek = function(base, path) {
  path = DAML.toArray(path)
  ...
  for(var i=0, l=path.length; i < l; i++) {
    ...
    for(var j=0, k=DAML.Pathfinders.length; j < k; j++) {
      pf = DAML.Pathfinders[j]
    ...
    if(!pf)
      return DAML.setError('No matching pathfinder was found')
  ...
  return todo.length ? todo[0] : ""
}




DAML.resolve_path_and_mod = function(path, base, fun) {
  if(typeof path == 'string') // just in case
    path = DAML.Parser.split_on(path, '.')
  ...
  var reducer = function(acc, value, words) {
    if(scalar_test(value))
      return DAML.setError("There's been a terrible mistake") || {}
...
DAML.recursive_extend = function(base, value) {
  for(var key in value) {
    ...
    DAML.recursive_extend(base[key], value[key])
  }
  ...
  for(var key in values) {
    // try { // NOTE: accessing e.g. input.selectionDirection throws an error, which is super-duper lame
      ...
      if(DAML.isBlock(val)) {
        ...
        new_values[key] = DAML.recursive_leaves_copy(val, fun, seen);
      } else {
        new_values[key] = fun(val);
      }
    // } catch(e) {DAML.onerror(e)}
  }
...
DAML.Parser.pipeline_string_to_tokens = function(string, quoted) {
  var tokens = []
    , P = DAML.Parser
    , strings = []
...
DAML.Parser.string_to_tokens = function(string) {
  ...
  if(chunk.length == string.length && chunk[0] == DAML.command_open) {
    ...
    do {
      ...
      if(chunk[0] == DAML.command_open) {
        result = DAML.Parser.pipeline_string_to_tokens(chunk, true)
      } else {
...
DAML.token_counter = 100000 // this is stupid // FIXME: make Rekey work even with overlapping keys
...
, execute: function(segment, inputs, dialect, prior_starter, process) {
    ...
    var processfun = function(value) {
      return DAML.execute_then_stringify(value, {}, process)
    }

    return DAML.dataTrampoline(inputs, processfun, DAML.string_concat, prior_starter)
  }
}
...
DAML.SegmentTypes.Fancy = {
  try_lex: function(string) {
    ...
    if(DAML.FancyRegex.test(string)) 
...
, munge_tokens: function(L, token, R) {
    ...
    if(!DAML.FANCIES[glyph]) {
      DAML.setError('Your fancies are borken:' + glyph + ' ' + token.value)
      return [L, R]
    }
    ...
    R.filter(function(future_segment) { return future_segment.type == 'Variable' 
     ...
     .forEach(function(future_segment) { 
       if(future_segment.value.prevkey)
         return DAML.setError('Pipeline variables may be set at most once per pipeline')
...
, execute: function(segment, inputs, dialect, prior_starter, process) {
    ...
    state[name] = DAML.deep_copy(inputs[0]) // NOTE: we have to deep copy here because cloning (via JSON) destroys blocks...
    
    return inputs[0]
  }
}

DAML.SegmentTypes.PortSend = {
...
, execute: function(segment, inputs, dialect, prior_starter, process) {
    ...
    if(port)
      ...
      DAML.setError('Invalid port detected')
    
    return inputs[0]
  }
}

DAML.SegmentTypes.Variable = {
  try_lex: function(string) {
    return string // this is never lexed
  }
, token_to_segments: function(token) {
...
, execute: function(segment, inputs, dialect, prior_starter, process) {
    ...
    return DAML.deep_copy(value) // NOTE: we have to deep copy here because cloning (via JSON) destroys blocks...
  }
}
...
, munge_segments: function(L, segment, R) {
    ...
    // DAML.replumb(R, new_key, function(future_segment, input) {
    // })
...
DAML.Dialect = function(commands, aliases) {
  this.commands = commands ? DAML.deep_copy(commands) : DAML.commands
  this.aliases = aliases ? DAML.clone(aliases) : DAML.ALIASES
  // this.parent = parent
}
...
// DAML.dialect_get_handler = function(dialect, handler) {
//   if(  handler 
//     && dialect.commands
...
// }
// 
// DAML.dialect_get_method = function(dialect, handler, method) {
//   if(  handler 
//     && method
//     && dialect.commands
//     && dialect.commands[handler]
...
DAML.spaceseed_add = function(seed) {
  ...
  seed = DAML.clone(seed) // keep the ref popo off our tails
  seed = DAML.sort_object_keys(seed)
  seed.state = DAML.sort_object_keys(seed.state)


  var sorted_stations = DAML.clone(seed.stations).sort(function(a,b) {return a - b})
  ...
  var sorted_subspaces = DAML.clone(seed.subspaces).sort(function(a,b) {return a - b})
  ...
  var route_clone = DAML.clone(seed.routes)
...
DAML.Port = function(port_template, space) {
  ...
  if(!pflav)
    return DAML.setError('Port flavour "' + flavour + '" could not be identified')
  ...
  if(port.space && !port.space.parent && !port.station && !port.subspace) {
    var outside_template = DAML.clone(port_template)
    delete outside_template['space']
    var outside_port = new DAML.Port(outside_template)
...
DAML.Space = function(seed_id, parent) {
  // the template a pointer to DAML.SPACESEEDS, which contains: id, dialect, state, ports, stations, subspaces, routes
  
  var seed = DAML.SPACESEEDS[seed_id]
    , self = this
  
  if(!seed)
    return DAML.setError('Invalid spaceseed')
  ...
  this.ports = seed.ports.map(function(port, index) {return new DAML.Port(port, self)})
  
  // add outs to ports
  seed.routes.forEach(function(route) {self.ports[route[0]-1].outs.push(self.ports[route[1]-1])})
  
  // add all my children
  this.subspaces = seed.subspaces.map(function(seed_id) {return new DAML.Space(seed_id, self)})
  ...
  // revise dialect
  this.dialect = DAML.DIALECTS.top // TODO: this probably isn't right, but the timing gets weird otherwise
  
  if(this.parent) {
    var parent_dialect = this.parent.dialect ? this.parent.dialect : DAML.DIALECTS.top
    this.dialect = new DAML.Dialect(parent_dialect.commands, parent_dialect.aliases)
...
DAML.Space.prototype.change_seed = function(seed_id) {
...
// DAML.Space.prototype.here_have_some_ports_thanks = function(child_ports) {
...
DAML.dataTrampoline = function(data, processfun, joinerfun, prior_starter, finalfun) {
  ...
  finalfun = finalfun || DAML.identity
...
DAML.list_set = function(total, value, key) {
...
DAML.execute_then_stringify = function(value, prior_starter, process) {
  if(DAML.isBlock(value)) {
    return DAML.TYPES['block'](value)(prior_starter, {}, process)
  } else {
...
DAML.ETC.niceifyish = function(value, whitespace) {
...
DAML.DIALECTS.top = new DAML.Dialect() // no params means "use whatever i've imported"
...
DAML.make_some_space = function(stringlike) {
  return DAML.make_spaceseeds(DAML.seedlikes_from_string(stringlike))
}

DAML.seedlikes_from_string = function(stringlike) {
  var seedlikes = {}
    , seed_offset = -1
    , prop_offset = -1
    , seed_name = ''
    ...
    if(this_offset == seed_offset) {
      if(seed_name) {
        if(seedlikes[seed_name]) {
          DAML.recursive_extend(seedlikes[seed_name], this_seed)
        } else {
      ...
      line.split('->').forEach(function(part, index) {
        ...
        if(part[0] == '@') {
          ...
          if(!route.length)
            ...
            if(!route[0] || !route[1]) {
              DAML.setError('Port not found in line: ' + line)
              route = []
            }
            else {
...
DAML.make_spaceseeds = function(seedlikes) {
  var seedmap = {}
    , newseeds = {}
  ...
  for(var seedkey in seedlikes) {
    ...
    for(var key in stations) {
      newseed.stations.push(DAML.Parser.string_to_block_segment(stations[key].value).value.id) // block id
    ...
    newseed.routes = 
      routes.map(function(route) {
        ...
        if(!one)
          DAML.setError('Invalid route: ' + route[0])
        if(!two)
          DAML.setError('Invalid route: ' + route[1])
    ...
    seedmap[seedkey] = DAML.spaceseed_add(newseed)
  }

...
DAML.import_fancy('_', {
  eat: function(token) {
    ...
    token.type = 'PipeVar'
    return [token] // this way we don't destroy the inputs and key and whatnot
    // return [new DAML.Token('String', token.value)] // FIXME: HERP DERP BERP
  }
})
...
DAML.Space.prototype.execute = function(ablock_or_segment, callback) {
  ...
  if(!callback) {
    callback = function(result) {
      DAML.setError("No callback sent to space.execute for result: " + DAML.TYPES['string'](result))
      // THINK: what should we do here?
      // console.log(result)
    }
  }
...
DAML.Process.prototype.next = function(callback) {
  ...
  return type.execute(segment, inputs, this.space.dialect, callback)
}
...
DAML.to_string_with_execution = function(value, callback) {
  if(DAML.isBlock(value)) {
    ...
    callback(DAML.TYPES['string'](value))
  }
}

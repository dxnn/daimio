...
D.import_models = function(new_models) {
  _.each(new_models, function(model, model_key) {
    if(!D.commands[model_key]) {
      ...
      _.extend(D.commands[model_key]['methods'], model['methods'])
    }
  })
}
...
D.run = function(daml, ultimate_callback, space) {
  if(!daml) return ""
  ...
  var result = space.execute(D.Parser.string_to_block_segment(daml), null, prior_starter)
  if(result === result)

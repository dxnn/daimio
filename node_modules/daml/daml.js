...
DAML.spaceseed_add = function(seed) {
  ...
  var sorted_stations = DAML.clone(seed.stations).sort()
  if(seed.stations != seed.stations.sort()) {
    ...
    while(seed.stations.length) {
      var max = Math.max.apply(null, seed.stations)
        , index = seed.stations.indexOf(max)
        , new_index = new_stations.length

      new_stations.push(max)

      if(space_index_to_ports[index]) {
    ...
    seed.stations = new_stations
  }

  var sorted_subspaces = DAML.clone(seed.subspaces).sort()
  if(JSON.stringify(seed.subspaces) != JSON.stringify(sorted_subspaces)) {
    // we have to manually sort them, because we have to keep their ports updated...
    var space_index_to_ports = {}
      , new_subspaces = []
      
    for(var key in seed.ports) {
      var port = seed.ports[key]
      if(!port.station) continue
      var item = space_index_to_ports[port.station]
      item ? item.push(port) : space_index_to_ports[port.station] = [port]
    }
    
    // OPT: O(n^2) yuck
    while(seed.subspaces.length) {
      var max = Math.max.apply(null, seed.subspaces)
        , index = seed.subspaces.indexOf(max)
        , new_index = new_subspaces.length

      new_subspaces.push(max)

      if(space_index_to_ports[index]) {
        space_index_to_ports[index].forEach(function(port) {
          port.station = new_index
        })
      }
  ...
  // these we can just sort. finally!

...
DAML.add_type('block', function(value) {
  if(DAML.isBlock(value)) {
    ...
    return function(callback, scope) {
      ...
      // TODO: don't use value.value.id directly below, because it might not be an ablock
      // TODO: how does this fit with parent processes and parallelization? 
      var space = DAML.SPACES.top
      space.REAL_execute(DAML.ABLOCKS[value.value.id], callback, scope) 
    }
  }
  else {
    value = DAML.stringify(value)
    return function(callback) {
      callback(value) // strings just fire away
    }
  }
})
...
, execute: function(segment, inputs, dialect, callback) {
    ...
    var new_cb = function(value) {
      if(counter < inputs.length) {
        output += value
        counter++
        DAML.to_string_with_execution(inputs[counter], new_cb)
      } else {
        callback(output)
      }
    }
    ...
    if(inputs.length) {
      DAML.to_string_with_execution(inputs[0], new_cb)
    } else {
      callback("")
    }

    // for(var i=0, l=inputs.length; i < l; i++) {
    //   output += DAML.to_string_with_execution(inputs[i], callback)
    // }
...
, munge_segments: function(L, segment, R) {
    ...
    // then, get the previous *top* segment's key
    ...
    if(prev_key === false) {
      if(L.length) {
        prev_key = L[L.length-1].key // THINK: first segment doesn't get marked as top, so we grab it here anyway
      } else {
        prev_key = -1 // nothing prior to __ so we give it -1 for Process operating context
      }
    }
    
    // finally replace our key with the previous key
    outputs.map(function(segment) {
      segment.inputs[segment.inputs.indexOf(my_key)] = prev_key
      // TODO: make this work for multiple connections (can those exist?)
    })
...
, execute: function(segment, inputs, dialect, callback, process) {
    ...
    if(!method) {
      // THINK: error?
      DAML.setError('You have failed to provide an adequate method')
...
, munge_tokens: function(L, token, R) {
    ...
    if(!new_tokens) {
      DAML.setError('The alias stares at you blankly')
      return [L, R]
    }
...
DAML.to_string_with_execution = function(value, callback) {
  if(DAML.isBlock(value)) {
    DAML.TYPES['block'](value)(callback)
  } else {
    callback(DAML.stringify(value))
  }
}

...
DAML.import_gateway_type = function(name, gtype) {
  if(DAML.GATEWAYTYPES[name])
    return DAML.setError('That gateway type has already been imported')
  
  if(!gtype || !gtype.addListener || !gtype.sendMessage)
    return DAML.setError('That gateway type is missing required properties')
  
  if(typeof gtype.addListener != 'function' || typeof gtype.sendMessage != 'function')
    return DAML.setError("That gateway type's properties are invalid")
  
  DAML.GATEWAYTYPES[name] = gtype
  return true
}

DAML.add_gateway = function(name, type, thing) {
  var gtype = DAML.GATEWAYTYPES[type]
  
  if(!gtype)
    return DAML.setError('Gateway type could not be found')
  
  if(DAML.GATEWAYS[name])
    return DAML.setError('That gateway has already been added')
    
  var gateway = Object.create(gtype)
  gateway.name = name
  gateway.type = type
  gateway.thing = thing
  
  return DAML.GATEWAYS[name] = gateway
}

DAML.import_gateway_type('dom-text', {
  sendMessage: function(value, callback) {
    var element = document.getElementById(this.thing)
    
    if(!element)
      return DAML.setError('That dom thing is not present')
  ...
  addListener: function(callback) {
    // THINK: what should this do? it could put the value on the channel immediately, but that's kind of silly...
  }
})

DAML.import_gateway_type('dom-submit', {
  sendMessage: function(value, callback) {
    var element = document.getElementById(this.thing)
...
DAML.import_gateway_type('space', {
  sendMessage: function(value, callback) {
    var space = DAML.SPACES[this.thing]
    
    if(!space)
      return DAML.setError('Space could not be found')
    
    // THINK: when would this ever get a real callback?
    if(!callback) {
      callback = function() {} // prevents 'no callback sent' error
    }
    
    return space.deliver(value, callback)
  },
  addListener: function(callback) {
    var space = DAML.SPACES[this.thing]
...
DAML.import_fancy('$', {
  eat: function(token) {
    token.type = 'Command'
    token.value = 'variable get path "' + token.value.slice(1) + '"'
    return [token]
  }
})
...
DAML.add_type('block', function(value) {
  if(DAML.isBlock(value)) {
    ...
    return function(callback, scope) {
      ...
      return space.REAL_execute(DAML.ABLOCKS[value.value.id], callback, scope) 
    }
  }
  else {
    value = DAML.stringify(value)
    return function(callback) {
      return callback(value) // strings just fire away
    }
  }
})
...
DAML.run = function(daml, callback, space) {
  if(!daml) return ""
  ...
  var new_cb = function(value) {
    var result = DAML.execute_then_stringify(value, callback)
    if(result === result) 
      callback(result)
  }
    
  space.execute(DAML.Parser.string_to_block_segment(daml), new_cb)
...
DAML.Vstack = [{}]
DAML.VARS = {}
DAML.execute = function(handler, method, params, callback, process) {
  var dialect = DAML.SPACES.top.dialect
    , real_handler = dialect.get_handler(handler)
    , real_method = dialect.get_method(handler, method)
  
  return real_method.fun.apply(real_handler, params, callback, process)
}
...
, execute: function(segment, inputs, dialect, callback) {
    return segment
    
    
    // var block = DAML.ABLOCKS[segment.value.id]
    //   , new_cb = function(value) {callback(value)}
    // 
    // return DAML.SPACES.top.REAL_execute(block, new_cb)
  }
}
...
, execute: function(segment, inputs, dialect, callback) {
    ...
    //     output += value
    //     counter++
    //     return DAML.execute_then_stringify(inputs[counter], thunk) // signals trampoline to continue or fail
    //   } else {
    //     return false // signals trampoline to call final callback
    //   }
    // }
    // 
    // var thunk = function(value, syncmode) {
    //   if(syncmode) 
    //   DAML.trampoline(fun, callback)
    // }
    // 
    // thunk()

    return DAML.dataTrampoline(inputs, DAML.execute_then_stringify, DAML.string_concat, previous_async_restarter)
    ...
    //     output += value
    //     counter++
    //     DAML.execute_then_stringify(inputs[counter], new_cb)
    //   } else {
    //     callback(output)
    //   }
    // }
    // 
    // DAML.execute_then_stringify(inputs[0], new_cb)
    
    // for(var i=0, l=inputs.length; i < l; i++) {
    //   output += DAML.execute_then_stringify(inputs[i], callback)
    // }
...
, munge_tokens: function(L, token, R) {
    ...
    last_replacement.top = token.top
    ...
    if(segment.value == '__') {
      ...
      if(new_key === segment.value) {
        if(L.length) {
          ...
          new_key = -1 // nothing prior to __ so we give it -1 for Process operating context
        }
      }
    }
    
    // finally replace our key with the previous key
    outputs.map(function(segment) {
      segment.inputs[segment.inputs.indexOf(my_key)] = new_key
...
, execute: function(segment, inputs, dialect, callback, process) {
    ...
    if(!errors.length) {
      return method.fun.apply(handler, params.concat(callback, process))
    } else {
    ...
    if(token.names) {
      ...
      for(var i=0, l=token.names.length; i < l; i++) {
        ...
        if(key == '__pipe__') {  
          ...
          last_replacement.inputs[lr_position] = value // always add the __pipe__
        }
        else if(key == '__alias__') {
...
DAML.Space = function(dialect, block, parent) {
...
DAML.Space.prototype.deliver = function(message, callback) {
  // execute the block, with the message loaded in as __
  var scope = {"-1": message} // TODO: find something better...
  this.execute(this.block, callback, scope)
}
...
DAML.Space.prototype.execute = function(ablock_or_segment, callback, scope) {
  var self = this
    , block = DAML.get_block(ablock_or_segment)
  
  if(!callback) {
    callback = function(result) {
      DAML.setError("No callback sent to space.execute for result: " + DAML.stringify(result))
      // THINK: what should we do here?
      // console.log(result)
    }
  }
  ...
  if(this.processes.length && this.only_one_process) {
    this.queue.push(function() {
      self.REAL_execute(block, callback, scope)
    })
  }
  else {
    self.REAL_execute(block, callback, scope)
...
DAML.Space.prototype.REAL_execute = function(block, callback, scope) {
  var process = new DAML.Process(this, block, callback, scope)
    , self = this
...
DAML.Space.prototype.cleanup = function(pid, value) {
  if(value !== undefined) {
    for(var i=0, l=this.listeners.length; i < l; i++) {
      this.listeners[i](value) // call the registered callbacks
    }
  }
...
DAML.Process = function(space, block, when_done, scope) {
  ...
  //     self.varset[key] = item
  //   })
  // }
  
  this.last_value = ""
  // this.varset = space.varset // for overriding?
  this.varset = scope || {} // process-level vars, like wiring, should be local to the process
}
...
DAML.Process.prototype.done = function() {
  ...
  this.space.cleanup(this.pid, this.last_value)
}
...
DAML.Process.prototype.run = function() {
  ...
  var my_cb = function(value) {
    self.last_value = value
    self.varset[self.current] = value // TODO: fix this it isn't general
    self.current++
    self.run()
  }
  
  while(this.block.segments[this.current]) {
    value = this.next(my_cb) // TODO: this is not a trampoline
    if(value !== value) {
      // TODO: NaN is a weird way to send the "I took the callback route" signal...
      return false
    }
    self.last_value = value
    self.varset[self.current] = value // TODO: fix this it isn't general
...
DAML.Process.prototype.next = function(callback) {
  var self = this
  ...
  if(wiring[key]) {
    inputs = wiring[key].map(function(index) {return DAML.isNice(varset[index]) ? varset[index] : false}) // THINK: why false?
  }
  
  return type.execute(segment, inputs, this.space.dialect, callback, this)
}
...
DAML.dataTrampoline = function(data, processfun, joinerfun, previous_async_restarter) {
  ...
  var inner = function() {
    while(++index < size) {
      value = processfun(data[index], my_async_restarter, result)
      if(value !== value) {
        asynced = true // we'll need to call if_async when we finish up
    ...
    if(asynced)
      return previous_async_restarter(result)

    return result
  }
  
  var my_async_restarter = function(value) {
    result = joinerfun(result, value)
    inner()
  }
...
DAML.execute_then_stringify = function(value, callback) {
  if(DAML.isBlock(value)) {
    return DAML.TYPES['block'](value)(callback)
  } else {

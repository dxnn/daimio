...
DAML.import_port_type('socket-user-add', {
  dir: 'in',
  outside_add: function() {
    var self = this
...
DAML.import_port_type('socket-user-remove', {
  dir: 'in',
  outside_add: function() {
    var self = this
...
DAML.make_some_space = function(stringlike) {
  ...
  return DAML.make_some_space2(seedlikes)
}


DAML.make_some_space2 = function(seedlikes) {
  var seedmap = {}
    , newseeds = {}
  ...
  for(var seedkey in seedlikes) {
    ...
    for(var key in ports) {
      newseed.ports.push({space: 0, flavour: ports[key][0], settings: {thing: (ports[key][1] || key)}, name: (ports[key][1] || key) }) // TODO: oh dear this should not be
    ...
    for(var key in stations) {
      ...
      // add my ports
      port_key_to_index[key + '.in'] = newseed.ports.push({space: 0, flavour: 'in', name: '_in', station: index})
      port_key_to_index[key + '.out'] = newseed.ports.push({space: 0, flavour: 'out', name: '_out', station: index})
      port_key_to_index[key + '.err'] = newseed.ports.push({space: 0, flavour: 'err', name: '_error', station: index})
    }

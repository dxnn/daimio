...
DAML.port_standard_exit = function(ship) { 
  if(this.space)
    this.outs.forEach(function(port) { port.enter(ship) })
  else
    this.outside_exit(ship) 
}
...
DAML.spaceseed_add = function(seed) {
  ...
  if(JSON.stringify(seed.stations) != JSON.stringify(sorted_stations)) {
    ...
    seed.stations.forEach(function(station, index) {
      var old_index = index + 1
        , new_index = sorted_stations.indexOf(station, last_offset[index]) + 1
  ...
  if(JSON.stringify(seed.subspaces) != JSON.stringify(sorted_subspaces)) {
    ...
    seed.subspaces.forEach(function(subspace, index) {
      var old_index = index + 1
        , new_index = sorted_subspaces.indexOf(subspace, last_offset[index]) + 1
...
DAML.seedlikes_from_string = function(stringlike) {
  ...
  stringlike.split("\n").forEach(function(line) {
    ...
    if(line[0] == '{') {
      try {this_seed.dialect = JSON.parse(line)} catch(e) {}
      return
    }

    if(/->/.test(line)) { // THINK: this should check line order (routes at end), as stations may use this symbol
      var route = []
    ...
    name = line.split(' ', 1)[0]
    value = line.slice(name.length).replace(/^\s+|\s+$/g, '')

    if(line[0] == '@') {
      name = name.slice(1)
      value = value ? value.split(' ') : [name]
      this_seed.ports[name] = value
      return
    }
    
    if(line[0] == '$') {
      try {this_seed.state[name.slice(1)] = JSON.parse(value)} catch(e) {}
      return
    }
    
    this_seed.stations[name] = value
  })

    ...
    methods: {
      ...
      add: {
        ...
        fun: function(sheet, type, id, data) {
          ...
          if(!sheet) return DAML.onerror('Invalid sheet id');
      ...
      remove: {
        ...
        fun: function(sheet, thing) {
          ...
          if(!thing) return DAML.onerror('Invalid thing id');
      ...
      set_template: {
        ...
        params: [
          ...
          {
            key: 'daml',
            desc: "A new daml template for the template",
            type: 'template',
            required: true,
          },
        ],
        fun: function(sheet, type, daml) {
          sheet = Daggr.sheets[sheet];
          if(!sheet) return DAML.onerror('Invalid sheet id');
          
          if(sheet.set_template(type, daml)) return sheet.id;
        },
      },
      ...
      set_data: {
        ...
        fun: function(sheet, thing, key, value) {
          ...
          // THINK: mirror objects are pretty weird... we either go whole hog and make them part of Daggr, or fiddle around out here in the handler somehow. Could attach a callback to setting things... but really, when will Daggr ever *set* values directly? but it does set them indirectly through moving sub-items. Could have a 'data' object in Daggr items that store x, y, and whatever else you put there through DAML. That's probably the cleanest way to get the values in and out. Then it's just a matter of triggering calls on Daggr when values change, and triggering calls in DAML when Daggr changes things... [but how do you differentiate?]
          
          // {value | > :@Daggr.{sheet}.{thing}.key}
          // DAML.recursive_insert(DAML.Vglobals, ['@Daggr', sheet.id, thing.id, key.split('.')], value);
